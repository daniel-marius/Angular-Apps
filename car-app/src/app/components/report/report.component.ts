...
export class ReportComponent implements OnInit {
  ...
  calculate(): void {
    for (const car of this.cars) {
      ...
      if (
        isNullOrUndefined(this.mostExpensive) ||
        car.price > this.mostExpensive.price
      ) {
        this.mostExpensive = car;
      }
      if (
        isNullOrUndefined(this.mostEfficient) ||
        car.engine.consumption < this.mostEfficient.engine.consumption
      ) {
